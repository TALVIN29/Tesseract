# PROJECT TESSERACT — ARCHITECTURAL AUDIT & EXECUTION PLAN
### Red Team Review + Ticket-Level Progress Plan
**Status:** Awaiting Approval | **Date:** 2026-06-08
...
## FINDING 1 — RACE CONDITION: Orphaned `job_id` on WebSocket Notify Failure

**Severity:** Critical  
**Location:** Deliverable 3 → Chemical Reaction Interface sequence
...
```
Plane → POST /cube/evaluate → Cube responds → Plane calls POST /sphere/notify → WebSocket push
```
...
- Cube: verdict written, `audit_ledger_entry_id` exists. ✓
- Plane: verdict received, notify call failed silently. ✗
...
**Patch 1A — Add `GET /api/sphere/verdict/{job_id}` reconciliation endpoint.**  
The Sphere must be able to poll for a verdict by `job_id` on WebSocket reconnect or after a configurable client-side timeout. This endpoint reads from the Audit Ledger (read-only path — does not touch the Cube directly).

**Patch 1B — Add `PROCESSING_TIMEOUT` UI pseudo-state.**  
...
**Patch 1C — Add exponential backoff to `POST /api/sphere/notify`.**  
...
## FINDING 2 — STATE MANAGEMENT: Redis Drop Leaves Sphere in Unrecoverable Limbo

**Severity:** High  
**Location:** Deliverable 1 → PLN-005 (Shock Absorption Queue) + Deliverable 3 → Stack
...
- **Message TTL** — a message can sit in the stream indefinitely if all Plane consumers are offline.
- **Dead Letter Queue (DLQ)** — if a consumer group acknowledges receipt but processing fails (e.g., n8n workflow crash mid-execution), the message is lost with no recovery path.
- **Poison message handling** — a malformed SegmentationPayload that passes the API gateway but fails Plane-side schema validation will be retried indefinitely by the consumer group, blocking the stream partition.
...
**Patch 2C — Add `job_id` status field to Sphere API response contract.**  
...
## FINDING 3 — API CONTRACT GAPS: Three Missing Definitions Break Layer Decoupling

**Severity:** Critical  
**Location:** Deliverable 3 → NearMissPacket Schema + CubeVerdictResponse Schema

### Gap 3A — `SegmentationPayload` is `$ref`'d but never defined
...
### Gap 3C — `NearMissPacket` has no `schema_version` field
...
# PART 2: EXECUTION ROADMAP — TICKET-LEVEL PROGRESS PLAN
...
## SPRINT 0 — ENVIRONMENT SETUP & SCHEMA RATIFICATION
### Duration: Days 1–3 (pre-Sprint 1, not counted in 12-week clock)
...
- [ ] **[TESS-000]** Initialize monorepo at `e:\Tesseract` with the following top-level structure:
  ```
  /cube          → Fastify API (Inner Cube)
  /planes        → n8n workflow exports + Node.js webhook handlers
  /sphere        → Next.js 15 frontend
  /infra         → Docker Compose, K8s manifests (stub), Prometheus/Grafana config
  /rules         → Cube YAML rule store (separate Git repo, dual-approval)
  /schemas       → OpenAPI 3.1 specs (NearMissPacket, SegmentationPayload, CubeVerdictResponse)
  /docs          → Runbooks, ADRs, audit outputs
  ```
- [ ] **[TESS-001]** Initialize `/rules` as a **separate** Git repository with branch protection:
...
- [ ] **[TESS-002]** Create `.env.example` file at repo root documenting all required environment variables:
  ```
  # Cube
  CUBE_PORT=3100
  ...
  ```
- [ ] **[TESS-003]** Add `.gitignore` entries: `.env`, `*.pem`, `*.key`, `node_modules/`, `.next/`, `n8n-data/`

---

#### E0-B: Docker Compose — Full Stack Local Environment

- [ ] **[TESS-004]** Create `infra/docker-compose.yml` with the following services:
...
- [ ] **[TESS-005]** Configure `postgres-cube` with a startup SQL script that:
...
- [ ] **[TESS-006]** Configure Redis with `appendonly yes` and `maxmemory-policy noeviction` to prevent silent message loss under memory pressure
- [ ] **[TESS-007]** Configure n8n environment variables in Docker Compose:
  - `N8N_ENCRYPTION_KEY` — required for credential storage
  - `N8N_BASIC_AUTH_ACTIVE=true`
  - `WEBHOOK_URL=http://n8n:5678/` — internal Docker network URL
- [ ] **[TESS-008]** Verify full stack starts cleanly: `docker compose up -d` → all 7 services `healthy`
- [ ] **[TESS-009]** Verify WORM policy: attempt a SQL `UPDATE` on `audit_ledger` → confirm `EXCEPTION` is raised

---

#### E0-C: Schema Ratification (Audit Patches Applied)

- [ ] **[TESS-010]** Define `SegmentationPayload` schema in `/schemas/segmentation-payload.yaml` per Audit Finding 3A. Must include all 7 required fields. Reviewed and merged to `main`.
- [ ] **[TESS-011]** Update `NearMissPacket` schema in `/schemas/near-miss-packet.yaml`:
  - Add `schema_version` field (Audit Finding 3C)
  - Add `signal_aggregation_hint` field (Audit Finding 3B) — `enum: [MAX_SEVERITY, FIRST_MATCH]`, default `MAX_SEVERITY`
  - Bump schema to `v1.0`
- [ ] **[TESS-012]** Update `CubeVerdictResponse` schema in `/schemas/cube-verdict-response.yaml`:
...
- [ ] **[TESS-013]** Update Sphere API `202 Accepted` response schema per Audit Finding 2C:
  - Add `status`, `ttl_seconds`, `poll_endpoint` fields
  - Document in `/schemas/sphere-enqueue-response.yaml`
- [ ] **[TESS-014]** Peer review all 4 schema files. Sign off from Architect + Compliance Lead. Merge to `main`.
...
- [ ] **[TESS-015]** Scaffold Fastify application in `/cube`:
  ...
  /cube
    /src
      ...
      /plugins      → auth, schema-validation, metrics
      /services     → risk-engine, audit-ledger, lockdown
      /schemas      → compiled JSON schemas from /schemas/*.yaml
    server.ts
    Dockerfile
  ```
...
- [ ] **[TESS-020]** Implement `AuditLedger` service class in `/cube/src/services/audit-ledger.ts`:
  - Single public method: `append(entry: AuditLedgerEntry): Promise<string>` — returns `audit_ledger_entry_id`
  - No `update()`, `delete()`, or `query()` methods on this service
- [ ] **[TESS-021]** Define `AuditLedgerEntry` TypeScript type:
  ```typescript
  type AuditLedgerEntry = {
    packet_id: string;           // UUID — from NearMissPacket
...
- [ ] **[TESS-023]** Implement `POST /api/v1/cube/evaluate` route:
  ...
  - Schema validation against `NearMissPacket` schema is active — malformed requests return `400` with validation errors
...
- [ ] **[TESS-025]** Create initial rule YAML structure in `/rules` repo:
  ...
  /rules
    /v1
      base-edges.yaml       → E1–E4 rules
      vertical-edges.yaml   → E5–E8 rules
      top-edges.yaml        → E9–E12 rules
      aggregation.yaml      → strategy: MAX_SEVERITY (Sprint 0 ratified default)
    manifest.yaml           → lists active rule files + version metadata
  ```
- [ ] **[TESS-026]** Implement rule loader service in `/cube/src/services/rule-loader.ts`:
  ...
  - Rules are loaded once at startup and cached in memory — no hot reload in V1
...
- [ ] **[TESS-027]** Implement `RiskEngine` service in `/cube/src/services/risk-engine.ts`:
  ...
  - Uses `aggregation_strategy: MAX_SEVERITY` — evaluates all signals, returns the highest-severity verdict
...
- [ ] **[TESS-028]** Implement WATCHLIST threshold evaluators (one function per trigger category):
  - [ ] Auth anomalies: ≥ 3 failed attempts in 15-minute window
  - [ ] Behavioral deviation: score > 2.5σ below cohort baseline
...
- [ ] **[TESS-029]** Implement SUSPEND threshold evaluators:
  - [ ] Auth failure: ≥ 5 consecutive failures
...
- [ ] **[TESS-030]** Implement NEAR_MISS threshold evaluators:
  - [ ] Velocity near-breach: ≥ 95% and < 100% of permitted limit, self-corrected
...
- [ ] **[TESS-031]** Wire `RiskEngine` to `POST /api/v1/cube/evaluate` — replace `501` stub with full evaluation + Audit Ledger write + response
- [ ] **[TESS-032]** Unit tests: 100% of threshold scenarios covered. Minimum 18 test cases (one per threshold trigger). Use a mock rule set — not the production `/rules` YAML.
...
- [ ] **[TESS-033]** Implement `LockdownService` in `/cube/src/services/lockdown.ts`:
  ...
  - On trigger: publishes `CUBE_LOCKDOWN` to Redis Pub/Sub, sets `CUBE_LOCKED` flag in Redis (TTL: none — manual clear only), logs at `FATAL` level
...
### SPRINT 3 — n8n Infrastructure & Core Workflows
**Duration:** Weeks 5–6
...
- [ ] **[TESS-042]** Build Segmentation-Driven Router as the master n8n workflow:
  ...
  - Step 1: Validate incoming payload against `SegmentationPayload` JSON schema (inline Code node). Malformed payloads → DLQ immediately.
...
- [ ] **[TESS-043]** Create routing ruleset JSON file `/planes/routing-rules/v1.json` with 10 routing rules covering all combinations of `user_tier` × `risk_band_last_known`:
  - `STANDARD` × `CLEAN` → `standard-onboarding-workflow`
  - `STANDARD` × `WATCHLIST` → `elevated-monitoring-workflow`
  - `ELEVATED` × `CLEAN` → `elevated-onboarding-workflow`
  - `ELEVATED` × `WATCHLIST` → `elevated-monitoring-workflow`
  - `ELEVATED` × `SUSPENDED` → `suspension-review-workflow`
  - `RESTRICTED` × `*` → `restricted-access-workflow`
  - `*` × `UNKNOWN` → `identity-verification-workflow`
...
- [ ] **[TESS-045]** Build Near Miss Event Emitter as a reusable n8n sub-workflow (callable from any domain workflow):
  ...
  - Step 3: Exponential backoff on failure: 100ms → 200ms → 400ms → 800ms → cap 5s
  - Step 4: On success — forward verdict + `job_id` to Sphere notify endpoint (also with backoff per Audit Finding 1C)
  - Step 5: On terminal failure after all retries — move original message to DLQ with reason `CUBE_UNREACHABLE`
...
### SPRINT 4 — External Integrations & Full Chain Validation
**Duration:** Weeks 7–8
...
- [ ] **[TESS-050]** Create OAuth2 credentials — **CRITICAL: select `Web Application` type, NOT Desktop Application:**
...
- [ ] **[TESS-051]** Configure Google Sheets credential in n8n:
  - Navigate to n8n Credentials → New → Google Sheets OAuth2 API
  - Enter `client_id` and `client_secret`
  - Click "Connect" — confirm OAuth2 authorization flow completes without `redirect_uri_mismatch` error
...
- [ ] **[TESS-052]** Build Google Sheets Sync Bridge n8n workflow:
  ...
  - Write operation: `APPEND` enriched row to audit sheet (never `UPDATE` — append-only to match Audit Ledger philosophy)
...
- [ ] **[TESS-054]** Implement Segmentation Context Injector as a reusable n8n Code node (importable by any domain workflow):
  ...
  - On validation failure: returns structured error `{ valid: false, errors: ValidationError[], job_id }` — workflow routes to DLQ
...
- [ ] **[TESS-056]** Write integration test script (`/planes/tests/e2e-chain-test.ts`):
  ...
  - Verifies each event: dequeued from Redis → routed by Segmentation Router → Near Miss Emitter fires → Cube evaluates → Audit Ledger entry written → Sphere receives WebSocket verdict push
...
- [ ] **[TESS-057]** Security review of all n8n webhook endpoints:
  ...
  - n8n is not publicly accessible — only reachable from within Docker internal network or via API gateway
...
### SPRINT 5 — Multi-Agent AI Layer & API Gateway
**Duration:** Weeks 9–10
...
- [ ] **[TESS-058]** Scaffold Next.js 15 App Router application in `/sphere`:
  ...
  /sphere
    /app
      /api
        /gateway/v1/route.ts     → Sphere inbound endpoint (enqueues to Redis)
        /sphere/notify/route.ts  → Plane → Sphere verdict push endpoint
        /sphere/verdict/[job_id]/route.ts → Reconciliation endpoint (Audit Finding 1A)
        /health/route.ts
      /dashboard                 → Sphere Health Monitor (operator-only)
      /layout.tsx
      /page.tsx
    /components
      /antigravity               → Bento Grid layout engine
      /verdicts                  → Verdict UI components
    /lib
      /agents                    → AI agent definitions
      /ws                        → WebSocket client
      /store                     → job_id registry (Zustand)
  ```
...
- [ ] **[TESS-062]** Define agent topology in `/sphere/lib/agents/topology.ts`:
  ...
  - **Classification Agents** (Claude Haiku 4.5): high-volume, narrow tasks — fraud signal tagging, entity type detection, cohort assignment. Run in parallel, feed structured tags back to Orchestrator before SegmentationPayload construction.
- [ ] **[TESS-063]** Implement Orchestrator Agent using Anthropic SDK with structured output:
  ...
  ```
  - Enable prompt caching on the system prompt (Anthropic SDK `cache_control: { type: "ephemeral" }`) — the system prompt contains the 20 `IntentType` definitions and is re-used across all requests
- [ ] **[TESS-064]** Implement Classification Agents (Haiku 4.5):
  ...
  - All three run in parallel via `Promise.all()` — Orchestrator awaits all three before constructing SegmentationPayload
- [ ] **[TESS-065]** Implement `job_id` registry in Zustand store (`/sphere/lib/store/job-registry.ts`):
  ...
  - Background timer: every 5 seconds, check all entries with `status: "QUEUED" | "PROCESSING"` older than 30 seconds → set `status: "PROCESSING_TIMEOUT"` → call `GET /api/sphere/verdict/:job_id` reconciliation endpoint (Audit Finding 1B)
...
- [ ] **[TESS-068]** Stress test full Sphere → Plane → Cube chain:
...
### SPRINT 6 — Antigravity Frontend & MVC Hardening
...
- [ ] **[TESS-069]** Implement `AntigravityGrid` layout engine in `/sphere/components/antigravity/grid.tsx`:
  ...
  - No component has a hardcoded `gridColumn` or `gridRow` prop — all positions are computed from `semantic_weight`
...
- [ ] **[TESS-070]** Implement `SEMANTIC_WEIGHT_MAP` as defined in Tech Stack Blueprint. Verify visual gravity test: mount a `CUBE_ORIGINATED_HIGH` component alongside `SPHERE_NATIVE` components — `CUBE_ORIGINATED_HIGH` must occupy top-left, maximum-scale position within 200ms of mount
- [ ] **[TESS-071]** Implement Framer Motion spring animations for all Antigravity components:
  ...
  - All position and scale changes use `layout` animation prop — never `transition: { type: "tween" }`
- [ ] **[TESS-072]** Build 8 core component types for the Bento Grid:
  - [ ] `VerdictCard` — displays Cube verdict with semantic weight class
  - [ ] `AgentStatusTile` — displays AI agent activity
  - [ ] `QueueDepthGauge` — live Redis queue depth
  - [ ] `SegmentationBadge` — user tier + cohort display
  - [ ] `JobStatusTracker` — real-time `job_id` status from Zustand store
  - [ ] `ProcessingTimeoutAlert` — surfaced on `PROCESSING_TIMEOUT` state (Audit Finding 1B)
  - [ ] `DQLEntryNotice` — surfaced when `job_id` enters DLQ state (Audit Finding 2C)
  - [ ] `SystemHealthBanner` — Sphere Health Monitor summary (operator-only)
...
- [ ] **[TESS-074]** Wire all user-initiated UI actions through the Orchestrator Agent:
  - Zero direct calls from UI components to `POST /api/gateway/v1` — all UI actions dispatch to a `useAgent()` hook that calls the Orchestrator
  - The Orchestrator constructs the SegmentationPayload and dispatches to the gateway
  - UI components receive state updates only from the Zustand `job_id` registry — never from direct API responses
- [ ] **[TESS-075]** Implement `useAgent()` hook:
  ...
  - Internally: calls Orchestrator Agent → dispatches to gateway → registers `job_id` in Zustand store

#### E3-G: MVC Acceptance Tests

- [ ] **[TESS-076]** Full end-to-end MVC acceptance test suite (`/sphere/tests/mvc-acceptance.test.ts`):
  - [ ] WATCHLIST scenario: submit event triggering auth anomaly → verify verdict surfaced in `VerdictCard` with `CUBE_ORIGINATED_MEDIUM` weight
  - [ ] SUSPEND scenario: submit event triggering 5 consecutive auth failures → verify verdict surfaced with `CUBE_ORIGINATED_HIGH` weight, session terminated
  - [ ] NEAR_MISS scenario: submit event at 96% velocity threshold → verify `VerdictCard` appears, `JobStatusTracker` shows `VERDICT_RECEIVED`
  - [ ] DLQ scenario: submit malformed payload → verify `DQLEntryNotice` surfaces after TTL expiry
  - [ ] Timeout scenario: simulate WebSocket disconnect during processing → verify `ProcessingTimeoutAlert` surfaces after 30 seconds → reconnect → verify reconciliation endpoint restores verdict
  - [ ] Lockdown scenario: simulate direct Sphere-to-Cube access attempt → verify `CUBE_LOCKDOWN` broadcast received → verify all Planes halt → verify Sphere enters maintenance state

#### E3-H: Security Penetration Test

- [ ] **[TESS-077]** Security penetration test checklist:
  - [ ] Direct Sphere-to-Cube HTTP call → confirm `403` + `CUBE_LOCKDOWN` fires
  - [ ] Malformed NearMissPacket injection (missing `packet_id`) → confirm `400` from Cube schema validation
  - [ ] Duplicate `packet_id` replay → confirm `already_processed: true` returned, no second Audit Ledger write
  - [ ] Expired JWT on Plane-to-Cube call → confirm `401` from Cube JWT middleware
  - [ ] OAuth2 token replay (Google Sheets) → confirm token rotation invalidates replayed token
  - [ ] SQL injection attempt via `entity_id_hashed` field → confirm parameterized query prevents injection
  - [ ] XSS via `behavioral_cohort` field rendered in UI → confirm Next.js escaping prevents execution
...
All 6 acceptance test scenarios pass. All 7 security pen test checks pass. All 3 runbooks merged to `/docs`. A user interaction in the Antigravity UI flows through the Orchestrator Agent → n8n Plane → Cube evaluation → WebSocket verdict push → `VerdictCard` in Bento Grid, with the full chain auditable in the Audit Ledger.
...
|---|---|---|---|
| **E0: Environment** | Sprint 0 | TESS-000 → TESS-014 | 15 tickets |
| **E1: Inner Cube** | Sprints 1–2 | TESS-015 → TESS-037 | 23 tickets |
| **E2: 2D Planes** | Sprints 3–4 | TESS-038 → TESS-057 | 20 tickets |
| **E3: Outer Sphere** | Sprints 5–6 | TESS-058 → TESS-080 | 23 tickets |
| **TOTAL** | | **TESS-000 → TESS-080** | **81 tickets** |
...
```
TESS-000 → TESS-003 (repo structure before anything)
TESS-004 → TESS-009 (Docker before all services)
TESS-010 → TESS-014 (schemas before Sprint 1 implementation)
TESS-015 → TESS-019 (Fastify scaffold before all Cube features)
TESS-020 → TESS-022 (Audit Ledger before Risk Engine)
TESS-027 → TESS-035 (Risk Engine before Lockdown wiring)
TESS-036 → TESS-037 (tests + deploy before Plane integration)
TESS-038 → TESS-041 (n8n + queue before workflows)
TESS-042 → TESS-044 (Router before domain workflows)
TESS-049 → TESS-051 (OAuth2 config before Sheets workflow)
TESS-055 (reconciliation endpoint) → required by TESS-065 (job_id registry timeout)
TESS-056 → TESS-057 (e2e test before security review)
TESS-058 → TESS-061 (Next.js scaffold + gateway before AI layer)
TESS-062 → TESS-065 (Agents before Zustand store)
TESS-069 → TESS-075 (Grid before acceptance tests)
TESS-076 (acceptance) → TESS-077 (pen test) → TESS-078–080 (runbooks)
```
...
3. **Sprint 0 start authorized?** Sprint 0 is infrastructure-only (Docker, schema ratification, repo setup) — no application logic. Confirm whether to proceed here first, or jump straight to Sprint 1 once approved.
